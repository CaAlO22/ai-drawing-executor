...
- **launch_app 自带观察窗口**(Qt 优先, tkinter 兜底, 都没有则 headless),
  ...
  - **左栏「当前工具」**: 当前工具的中文名与英文名、颜色 / 粗细 / 不透明度 /
    容差 / 命中半径等设置, 以及画布尺寸、已画步数、已导出次数;
  - **中栏**: 只读画布实时画面;
  - **右栏「历史记录」**: 每一次工具调用映射成**自然语言**, 格式为
    `(颜色) 工具 动作 (路径/形状或坐标)` —— 路径只写**起点→终点**,
    不铺完整路径。例如:

    ```text
    选好工具：硬笔（颜色 #1f4e79，粗细 40）        ← 灰色提示行
    第 1 步：#1f4e79 硬笔画了 (150,700)→(750,700)（闭合）
    看了一眼当前画面
    选好工具：油漆桶（颜色 #ffd966，容差 20）
    第 2 步：#ffd966 油漆桶选择 (480,520) 填充
    撤销了上一步（action_000003）
    第 3 步：#c00000 软笔画了 (300,700)→(600,700)
    第 4 步：#c00000 软笔画了 圆形                  ← 形状报名称, 不报参数
    笔画橡皮：这个位置没有命中任何笔画            ← 失败翻成人话
    选择工具 执行失败：Invalid tool. Expected one of: ...
    完成本次绘画：观察窗三栏（新文案）            ← 紫色结束行
    ```
